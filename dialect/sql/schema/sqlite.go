...
func (d *SQLite) atUniqueC(t1 *Table, c1 *Column, t2 *schema.Table, c2 *schema.Column) {
	...
	atUniqueColumn(t1, c1, t2, c2, fmt.Sprintf("%s_%s_key", t2.Name, c1.Name), d.atImplicitIndexName)
}
...
func (d *SQLite) atImplicitIndexName(idx *Index, t1 *Table, c1 *Column) bool {
	...
	return atImplicitIndexSuffix(idx.Name, fmt.Sprintf("sqlite_autoindex_%s_", t1.Name), 1)
}

func (d *SQLite) atIncrementC(t *schema.Table, c *schema.Column) {
	atIncrementColumn(t, c, c.Default != nil, reflect.TypeOf(&sqlite.AutoIncrement{}), &sqlite.AutoIncrement{})
}
