...
func (d *MySQL) atUniqueC(t1 *Table, c1 *Column, t2 *schema.Table, c2 *schema.Column) {
	...
	atUniqueColumn(t1, c1, t2, c2, c1.Name, func(idx *Index, _ *Table, c1 *Column) bool {
		return d.atImplicitIndexName(idx, c1)
	})
}

func (d *MySQL) atIncrementC(t *schema.Table, c *schema.Column) {
	atIncrementColumn(t, c, c.Default != nil, reflect.TypeOf(&mysql.AutoIncrement{}), &mysql.AutoIncrement{})
}
...
func (d *MySQL) atImplicitIndexName(idx *Index, c1 *Column) bool {
	...
	// MySQL names implicit indexes as "<col>_2", "<col>_3", etc.
	// (starting from 2, since "<col>" is taken by the exact match).
	return atImplicitIndexSuffix(idx.Name, c1.Name+"_", 2)
}
...
func (d *MySQL) atIndex(idx1 *Index, t2 *schema.Table, idx2 *schema.Index) error {
	...
	if err := atIndexParts(idx1, t2, idx2, func(c1 *Column, _ *schema.Column) ([]schema.Attr, error) {
		if v, ok := prefix[c1.Name]; ok {
			return []schema.Attr{&mysql.SubPart{Len: int(v)}}, nil
		}
		return nil, nil
	}); err != nil {
		return err
